...
def run_deep(X, y, idx, test_size,
    ...
    return print_str, test_results, best_val_mlp
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--NumOfRuns", help="dataframe save path")
    ...
    X = X_df.to_numpy()
    y_total = df_to_run.iloc[:, 14:].to_numpy()
    ...
    results_df = dict()
    results_df['name'] = list()
    for idx in range(5):
        results_df[str(idx)] = list()

    for col, x in zip(X_df.columns, X.T):

        results_df['name'].append(col + '_loss')
        results_df['name'].append(col + '_corr')
        results_df['name'].append(col + '_r2')
        # x = x.reshape(-1, 1)
        x = X_df.drop(columns=col).to_numpy()

        for idx in range(5):
            y = copy.deepcopy(y_total[:, idx])
            result = dict(loss=list(), corr=list(), r2=list())
            # best_mlp = None
            # best_loss = np.inf
            # best_corr = -np.inf
            # best_r2 = -np.inf
            for i in tqdm(range(n_runs)):
                ...
                result['corr'].append(res['corr'])
                result['r2'].append(res['r2'])

            results_df[str(idx)] += [np.mean(result['loss']), np.mean(result['corr']), np.mean(result['r2'])]
        ...
        #                         '%.2E' % Decimal(p_r2), sig_r2]
            print(f'col {col} & idx {idx}'
                  f'\n loss {np.mean(result["loss"])} | corr {np.mean(result["corr"])} | r2 {np.mean(result["r2"])}')
