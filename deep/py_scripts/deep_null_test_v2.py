...
def run_deep(X_train, X_val, X_test, y_train, y_val, y_test,
             idx,
             train_batch_size,
             val_batch_size,
             learning_rate,
    ...
    # test resutls
    inp_test, target_test = torch.tensor(X_test).float(), torch.tensor(y_test).float()
    target_test = target_test.reshape((target_test.shape[0], 1))
    output_test = best_val_mlp(inp_test)
    output_test_numpy = torch.Tensor.numpy(torch.detach(output_test)).reshape(1, -1)[0]
    target_test_numpy = torch.Tensor.numpy(torch.detach(target_test)).reshape(1, -1)[0]

    test_corrcoef = np.corrcoef(target_test_numpy, output_test_numpy)[0, 1]
    test_r_squared = r2_score(target_test_numpy, output_test_numpy)
    test_loss = loss_function(output_test, target_test).item() / len(y_test)
    test_results = dict(loss=test_loss, corr=test_corrcoef, r2=test_r_squared)
    ...
    return print_str, test_results, best_val_results, best_val_mlp
...
if __name__ == "__main__":
    ...
    X_train_total, _X_val_total, y_train_total, _y_val_total = train_test_split(X_total, y_total, test_size=29)
    X_val_total, X_test_total, y_val_total, y_test_total = train_test_split(_X_val_total, _y_val_total,
                                                                            test_size=10)
    ...
    for idx in range(5):
        ...
        for i in tqdm(range(n_shuff)):
            ...
            _, test_res, _, mlp = run_deep(X_train, X_val, X_test, y_train, y_val, y_test,
                                           ...
                                           tensorboard=False,
                                           do_print=False)
            alternative_res['loss'].append(test_res['loss'])
            alternative_res['corr'].append(test_res['corr'])
            alternative_res['r2'].append(test_res['r2'])
            if test_res['loss'] < best_loss:
                best_loss = copy.deepcopy(test_res['loss'])
        ...
        for i in tqdm(range(n_shuff)):
            ...
            tensorboard_path = os.path.join('tensorboard5/', str(idx) + '_' + str(int(round(time.time() % 1, 5) * 1e5)))
            _, test_shuf_r, _, _ = run_deep(X_train, X_val, X_test, y_train, y_val, y_test,
                                            idx,
                                            train_batch_size=10,
                                            val_batch_size=5,
                                            learning_rate=1e-3,
                                            ...
                                            tensorboard=False,
                                            do_print=False)
            null_res['loss'].append(test_shuf_r['loss'])
            null_res['corr'].append(test_shuf_r['corr'])
            null_res['r2'].append(test_shuf_r['r2'])
